...
class RequestConsiderController extends Controller
{
    public function getRequest(Request $request)
    {
        ...
        //         'ooap_tbl_requests.req_endmonth',
        ...
        //     ->leftjoin('ooap_mas_divisions', 'ooap_tbl_requests.req_div', '=', 'ooap_mas_divisions.division_id');
        ...
        $data = OoapTblRequest::getTbDatas(NULL, auth()->user()->emp_type, $request->req_acttype, $request->req_year, $request->status, $request->serachbox);
        ...
        $data = $data
            ...
            ->orderby('ooap_tbl_requests.status', 'asc')
            ...
            ->addColumn('edit', function ($data) {
                ...
                if ($data->status == 1) {
                    if ($data->acttype_id == 1) {
                        $button .= '<a href="/request/consider/' . $data->req_acttype . '/' . $data->req_id . '/detail"><i class="icon wb-pencil" aria-hidden="true" title="แก้ไข"></i></a>';
                    } else {
                        $button .= '<a href="/request/consider/' . $data->req_acttype . '/' . $data->req_id . '/detail"><i class="icon wb-pencil" aria-hidden="true" title="แก้ไข"></i></a>';
                    }
                } else {
                    if ($data->acttype_id == 1) {
                        $button .= '<a href="/request/consider/' . $data->req_acttype . '/' . $data->req_id . '/detail"><i class="icon wb-eye" aria-hidden="true" title="ดูข้อมูล"></i></a>';
                    } else {
                        $button .= '<a href="/request/consider/' . $data->req_acttype . '/' . $data->req_id . '/detail"><i class="icon wb-eye" aria-hidden="true" title="ดูข้อมูล"></i></a>';
                    }
