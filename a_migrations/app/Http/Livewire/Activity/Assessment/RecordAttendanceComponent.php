...
class RecordAttendanceComponent extends Component
{
    use WithFileUploads;

    public $resultPopCheck = [];
    public $resultAcc;
    ...
    public $show_ac = [], $show_no_ac = [], $show_all = [], $c_status = false;
    ...
    public $course_trainer_rate, $act_amount, $show_emer_or_train, $act_foodrate, $act_materialrate, $food_r, $material_r, $trainer_r,$acttype2;
    ...
    public function render()
    {
        ...
        return view('livewire.activity.assessment.record-attendance-component');
    }
    ...
    {
        if ($val != '') {
            $pull_db = OoapTblPopulationCheckin::all()
                ->where('act_number', '=', $this->reqform_no_list[$this->act_id])
                ->where('in_active', '=', false);

            $this->resultPopCheck = $pull_db->groupBy('check_date')->toArray();
            if ($this->resultPopCheck) {
                foreach ($this->resultPopCheck as $key => $val) {
                    $tb = 0;
                    $ta = 0;
                    $a = $pull_db->where('check_status', '=', 1)->where('check_date', '=', $key)->toArray();
                    if ($a) {
                        $ta = count($a);
                        $this->show_ac[$key] = $ta;
                    } else {
                    ...
                    $b = $pull_db->where('check_status', '=', 0)->where('check_date', '=', $key)->toArray();
                    if ($b) {
                        $tb = count($b);
                        $this->show_no_ac[$key] = $tb;
                    } else {
                        $this->show_no_ac[$key] = 0;
                    }

                    $this->show_all[$key] = $ta + $tb;
                }
            }
        }
    }
    ...
    // protected $listeners2 = ['redirect-to-p-1' => 'redirect_to_p_1'];

    public function redirect_to()
    {
        return redirect()->route('activity.recordattendance.index');
    }

    // public function redirect_to_p_1()
    // {
    //     // return redirect()->route('activity.recordattendance.index');
    // }
