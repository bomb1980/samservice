...
$this->title  = 'แก้ไขหน่วยงาน' . Yii::$app->params['prg_ctrl']['pagetitle'];
...
<?php
$data = \app\models\lkup_branch::getEditData($id); //var_dump($data);exit;
$items = (array) $data[0];
...
if (!$superadmin) {  //ถ้าไม่ใช่แอดมินสูงสุด
	if ($user_role == 2) {
		$datassobranch_code = $items["ssobranch_code"];
	...
	<div class="page-header">
		...
		<div class="page-header-actions">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item atcitve">แก้ไขหน่วยงาน</li>
			</ol>
		</div>
	</div>
	<div class="page-content container-fluid">
		<div class="panel">
			<div class="panel-heading">
				<h3 class="panel-title">แก้ไขหน่วยงาน</h3>
			</div>
				...
				<form class="frmadd" method="post" name="frmadd" id="frmadd" enctype="multipart/form-data">
					<div class="row row-lg">
						<div class="col-md-12 col-lg-9">
							<div class="image-wrap">
								<img class="image rounded" src="<?php echo $folder . '/' . $items['cover']; ?>" style="object-fit:contain; width:100%; height:138px; border-style: solid;">
							</div>
						</div>
					</div>
					<div class="row row-lg">
					...
					<div class="row row-lg">
						<div class="col-md-4">
							...
							<input type="text" class="form-control" id="branchcode" name="branchcode" placeholder="รหัสหน่วยงาน" value="<?php echo $items['ssobranch_code']; ?>" disabled>
						</div>
					</div><br>
					<div class="row row-lg">
						<div class="col-md-4">
							<h4 class="example-title">ชื่อหน่วยงาน</h4>
							<input type="text" class="form-control" id="name" name="name" placeholder="ชื่อหน่วยงาน" value="<?php echo $items['name']; ?>">
						</div>
					</div><br>
					<div class="row row-lg">
						<div class="col-md-4">
							<h4 class="example-title">ชื่อย่อหน่วยงาน</h4>
							<input type="text" class="form-control" id="shortname" name="shortname" placeholder="คำย่อ" value="<?php echo $items['shortname']; ?>">
						</div>
					</div><br>
					<div class="row row-lg">
						<div class="col-md-4">
							...
							<select class="form-control" data-plugin="select2" id="ssobranch_type_id" name="ssobranch_type_id">
								...
								foreach ($branch_type as $dataitem) {
									$select =  $items['ssobranch_type_id'] == $dataitem['id'] ? "selected" : "";
					...
					<div class="row row-lg">
						<div class="col-md-12">
							...
							<div class="comments-add media">
								<textarea id="edit" name="content"><?php echo stripslashes($items['description']); ?></textarea>
							</div>
						</div>
						<script>
							$(function() {
					...
					<div class="row row-lg">
						<div class="col-md-12">
							...
							<input type="text" class="form-control" id="address" name="address" placeholder="ที่อยู่" value="<?php echo $items['address']; ?>">
						</div>
					</div><br>
					<div class="row row-lg">
						<div class="col-md-12">
							<h4 class="example-title">หมายเลขโทรศัพท์ติดต่อ</h4>
							<input type="text" class="form-control" id="telno" name="telno" placeholder="โทรศัพท์" value="<?php echo $items['telno']; ?>">
						</div>
					</div><br>
					<div class="row row-lg">
						<div class="col-md-12">
							<h4 class="example-title">ลิงค์แผนที่</h4>
							<input type="text" class="form-control" id="location" name="location" placeholder="ลิงค์" value="<?php echo $items['location']; ?>">
						</div>
					</div>					
			</div><br>
		...
		$('button[id=btnadd]').click(function() {
			...
			elems.forEach(function(el) {
				el.onsubmit = function(e) {
					...
					$.ajax({
						url: "<?php echo Yii::$app->urlManager->createAbsoluteUrl("branch/saveedit"); ?>",
						method: "POST",
						...
						success: function(data) {
							if (data.status == 'success') {
								...
								window.location.href = '<?php echo Yii::$app->params['prg_ctrl']['domain'] . '/branch/viewdata/' . $id; ?>';
							} else {
								alert(data.msg);
