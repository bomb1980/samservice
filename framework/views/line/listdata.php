...
<div class="page">
  ...
  <div class="page-header">
    <h1 class="page-title">รายการ ข้อความ</h1>
  </div>
          ...
          <div class="col-lg-auto" style="margin: 0 auto;max-width: 1200px;">
            ...
            <div class="form-inline">
              ...
              <div class="form-group form-material">
                ...
                if ($btnVisible) {
                ?>
                  <button type="button" onclick="location.href='<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/message"); ?>';" class="btn btn-primary waves-effect waves-classic ml-5" name="btnCreate" id="btnCreate" title="สร้างข้อความ">สร้างข้อความ</button>
                <?php
                }
                ?>
              </div>
          ...
          <table id="productTable1" class="table table-striped table-hover table-bordered tableUpdated">
            <thead>
              <tr>
                ...
                <th style='width:20%'>สำหรบประกันสังคม</th>
...
<script type="text/javascript">
  ...
  function checkFields(type_id) {
    ...
    dataTable = $(tbel).DataTable({
      ...
      "serverSide": true,
      "ajax": {
        url: "<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/listmessage"); ?>",
        "data": {
          'search': search,
  ...
  function ajax_setdefault(_this, id) {
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.msg == 'success') {
          alert('ต้องการตั้งค่าให้เป็นเมนูเริ่มต้นในระบบไลน์เรียบร้อย');
          checkFields(1);
        } else {
          alert(data.msg);
          $(_this).removeAttr('disabled');
  ...
  function ajax_sendmessage(_this, id) {
    var result = confirm("ต้องการส่งข้อความไปหาสมาชิก ?");
    if (!result) {
      $(_this).removeAttr('disabled');
      return;
    }
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/pushmessage"); ?>",
      data: {
        'YII_CSRF_TOKEN': '<?php echo Yii::$app->request->csrfToken; ?>',
        '<?=Yii::$app->request->csrfParam?>': '<?=Yii::$app->request->getCsrfToken()?>',
        'id': id,
      ...
      success: function(data) {
        if (data.msg == 'success') {
          alert('ส่งข้อความเข้าไปในระบบไลน์เรียบร้อย');
          checkFields(1);
        } else {
          alert(data.msg);
          $(_this).removeAttr('disabled');
  ...
  function ajax_deldata(_this, id) {
    ...
    var result = confirm("ต้องการลบชุดข้อความ ?");
    if (!result) {
      $(_this).removeAttr('disabled');
      return;
    }
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/delete_message"); ?>",
      data: {
        'YII_CSRF_TOKEN': '<?php echo Yii::$app->request->csrfToken; ?>',
        '<?=Yii::$app->request->csrfParam?>': '<?=Yii::$app->request->getCsrfToken()?>',
        'content_id': id,
