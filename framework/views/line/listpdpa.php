...
<div class="page">
  ...
  <div class="page-header">
    <h1 class="page-title">รายการ PDPA</h1>
  </div>
          ...
          <div class="col-lg-auto" style="margin: 0 auto;max-width: 1200px;">
            ...
            <div class="form-inline">
              <div class="form-group form-material">
                <input type="text" class="form-control" id="txtSearch" name="txtSearch" placeholder="ข้อความ PDPA หรือ เลขเวอร์ชั่น" autocomplete="off">
              </div>
              <div class="form-group form-material">
                ...
                if ($btnVisible) {
                ?>
                  <button type="button" onclick="location.href='<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/pdpa"); ?>';" class="btn btn-primary waves-effect waves-classic ml-5" name="btnCreate" id="btnCreate" title="สร้าง PDPA">สร้าง PDPA</button>
                <?php
                }
                ?>
              </div>
          ...
          <table id="productTable1" class="table table-striped table-hover table-bordered tableUpdated">
            <thead>
              <tr>
                <th style="width: 2%;">#</th>
                <th></th>
                <th scope="col" style='width:30%'>ข้อความ</th>
                <th scope="col" style='width:5%'>เวอร์ชั่น</th>
...
<script type="text/javascript">
  ...
  function checkFields(type_id) {
    ...
    dataTable = $(tbel).DataTable({
      ...
      "serverSide": true,
      "ajax": {
        url: "<?php echo Yii::$app->urlManager->createAbsoluteUrl("/line/listbackpdpa"); ?>",
        "data": {
          'search': search,
      ...
      createdRow: function(row, data, index) {
        var pageUrl = '<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/pdpa") ?>/' + data[1];
  ...
  function ajax_deldata(_this, id) {
    ...
    var result = confirm("ต้องการลบข้อความ PDPA (ถ้าเคยมีการใช้งานแล้วอาจจะไม่สำเร็จ) ?");
    if (!result) {
      $(_this).removeAttr('disabled');
      return;
    }
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo Yii::$app->urlManager->createAbsoluteUrl("line/delete_pdpa"); ?>",
      data: {
        '<?= Yii::$app->request->csrfParam ?>': '<?= Yii::$app->request->getCsrfToken() ?>',
        'content_id': id,
      },
