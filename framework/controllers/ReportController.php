...
?>
	...
	public function actionInsurerlogin()
	{
		...
		if (!empty($_GET["id"])) {
			...
			if (is_numeric($id)) {
				...
				if ($isAddmin) {
					...
					return $this->render('insurerlogin', array('data' => $rows, "user_role" => $user_role, "user_id" => $user_id, 'id' => $id));
				} else {
					if (Yii::$app->request->urlReferrer) {
						$redirect = Yii::$app->request->urlReferrer;
					} else {
			...
			if ($isAddmin) {
				return $this->render('insurerlogin', array('data' => null, "user_role" => $user_role, "user_id" => $user_id,));
			}
		}
	}
