...
class ServicedevController extends Controller
{
    ...
    public function actionEsv()
    {
        ...
        switch (strtolower($cmd)) {
            ...
            case strtolower("getS40Cntr"):
                ...
                break;
            case strtolower("getS40Cntr2"):
                return $this->getS40Cntr2();
                break;
    ...
    function getEmployeeOldAge()
    {
        ...
        $m = new \Memcached();
        $m->addServer('localhost', 11211);
        //$m->delete('getEmployeeOldAge' . $ssoNo);exit;
        $Fetch  = False;
    ...
    //ข้อมูลเงินสบทบมาตรา 40 แบบมีผลประโยชน์
    function getS40Cntr2()
    {
        $ssoNo = null;
        if (!empty($_REQUEST['ssoNo'])) $ssoNo = $_REQUEST['ssoNo'];
        if (is_null($ssoNo)) {
        ...
        $m->addServer('localhost', 11211);
        $Fetch  = False;
        //$m->delete('getS40Cntr2' . $ssoNo . $year);exit;

        $getS40Cntr2 = $m->get('getS40Cntr2' . $ssoNo . $year); //var_dump($getS40Cntr2);exit;
        if ($getS40Cntr2) {

            $today = new \DateTime("today"); // This object represents current date/time with time set to midnight

            $match_date = \DateTime::createFromFormat("Y-m-d H:i:s", $getS40Cntr2[0]);
            $match_date->setTime(0, 0, 0); // set time part to midnight, in order to prevent partial comparison

            $diff = $today->diff($match_date);
            $diffDays = (int)$diff->format("%R%a"); // Extract days count in interval

            $callh = \DateTime::createFromFormat("Y-m-d H:i:s", $getS40Cntr2[0]);
        ...
        if ($Fetch == True) {
            ...
            if ($getinfo) {
                //var_dump($getinfo);
                $m->set('getS40Cntr2' . $ssoNo . $year, [date("Y-m-d H:i:s"), $getinfo], 86400); // หมดอายุ 1 วัน
                return $getinfo;
            } else {
                return [
                    'code' => 'WS500',
    ...
    function ChangeHospital()
    {
        ...
        if ($Fetch == True) {
            ...
            $client = new \SoapClient("https://uatwsg.sso.go.th/services/ChangeHospital?WSDL");
    ...
    public function actionAuthtoken($ssoNo = "")
    {
        ...
        $command = $conn->createCommand($sql);
        $command->bindValue(":su_idcard", $ssoNo);
        $rs = $command->queryAll();var_dump($rs);exit;
        if (count($rs) > 0) {
