		...
		*/
		...
		$sql = "
			SELECT * FROM (
				SELECT
					...
					trn_m39notifications.notitext,
					trn_m39notifications.notitype,
		...
		foreach ($rows as $dataitem) {
			...
			*/
			...
			$sql = "SELECT name FROM mas_m39notitype WHERE id='".$dataitem['notitype']."' AND status=1 ";
			$command = $conn->createCommand($sql);
			$rtype = $command->queryAll();
			if (count($rtype) > 0) {
				$dataitem['notitype'] = $rtype[0]['name'];
			} else {
				$dataitem['notitype'] = "";
			}
			...
			//$dataitem['btn_line'] = $btnpush . $btnDefault;
			$dataitem['notitext'] = mb_strimwidth(strip_tags($dataitem['notitext']), 0, 200, "...");
			$dataitem['btn'] = $strbtn;
			/*
			$strswitch = '
				<div class="col-md-2 mr-auto">
